       ...
       <script type="text/javascript">
          ...
          //得到搜索条中选择的文件类型
           ...
           function Search() {
               ...
               if (show == 1) {
                   document.getElementById("iframe_Filelist").src = "/Search/SearchTableList?cid=" + id + "&type=" + type + "&sid=1" + "&search=" + search;
               }
               else
                    document.getElementById("iframe_Filelist").src = "/Search/Searchfilelist?sid=0" + "&cid=" + id + "&type=" + type + "&search=" + search;
           ...
           $(function () {
               ...
               $(".indexTop .searchArea .docs  li ").eq(0).removeClass('current');
               if (FormatType == "pic") {
                   type = 2;
                   $(".indexTop .searchArea .docs  li ").eq(4).addClass('current');
               }
               else if (FormatType == "doc") {
                   type = 1;
                   $(".indexTop .searchArea .docs  li ").eq(1).addClass('current');
               }
               else if (FormatType == "pdf") {
                   type = 8;
                   //type = 1;
                   $(".indexTop .searchArea .docs  li ").eq(3).addClass('current');
               }
               else if (FormatType == "ppt") {
                   type = 7;
                   $(".indexTop .searchArea .docs  li ").eq(2).addClass('current');
               }
               else if (FormatType == "vedio") {
                   type = 3;
                   $(".indexTop .searchArea .docs  li ").eq(5).addClass('current');
               }
               else {
                   type = 0;
           ...
           function getchannel(cid) {
               if (show == 1) {
                   ...
                   document.getElementById("iframe_Filelist").src = "/Search/SearchTableList?cid=" + cid + "&type=" + typeType + "&sid=1" + "&search=" + search;
                   id = cid;
                   show = 1;
               }
               else {

                   document.getElementById("iframe_Filelist").src = "/Search/Searchfilelist?cid=" + cid + "&type=" + typeType + "&sid=0" + "&search=" + search ;
                   id = cid;
                   show = 0;
           ...
           function GetTypeOfFile(type) {
               ...
               if (show == 1) {
                   ...
                   document.getElementById("iframe_Filelist").src = "/Search/Searchfilelist?cid=" + id + "&type=" + type + "&sid=0" + "&search=" + search;                                         
                   typeType = type;
                   show = 0;
               }
           ...
           function ShowChange() {
               document.getElementById("iframe_Filelist").src = "/Search/Searchfilelist?sid=0" + "&cid=" + id + "&type=" + typeType + "&search=" + search;
               document.getElementById("list1").src = "../../images/ico3.jpg";
               document.getElementById("list0").src = "../../images/ico4.jpg";
               show = 0;
           }   
           function ShowList() {
               document.getElementById("iframe_Filelist").src = "/Search/SearchTableList?sid=1" + "&cid=" + id + "&type=" + typeType + "&search=" + search;
    ...
    <div class="indexCont">
    	...
    	<div class="mainCont">
           ...
           <div style=" clear:both;"></div>
            <div style="float:right; margin-right:20px; margin-bottom:-20px; width:100px; height:30px;">
                    <p style=" float:left;" >
                        显示方式：</p>
                   
                        <img id="list0" alt="" src="../../images/ico4.jpg" style="cursor: pointer; border:0px;" title="矩阵显示" onclick="ShowChange();" />
                        <img id="list1" alt="" src="../../images/ico3.jpg" style="cursor: pointer; border:0px;" title="列表显示"  onclick="ShowList();" /><%--</a>--%>
                        </div>
              </div>
              <div style=" clear:both;"></div>
            <div class="menuFootLine"> </div>
            <%--<ul class="Resources">
            	      <%List<ZK.Model.View_AllFileList> allfilemodel = (List<ZK.Model.View_AllFileList>)ViewData["FileInfo"]; %>
                    <%foreach (var item in allfilemodel)
                      {%>
                      <li>
                    <img src="<%=item.imageURL%>" width="80" height="90" />
                    <div class="resTitle">
                         <%if (item.fileName.ToString().Length > 10)
                            ...
                            <%=item.fileName.ToString().Substring(0, 10)%>...<%}
                                  ...
                                  <%=item.fileName%>                      
                    <div class="resAttr">
                        <span id="resPage"><%=item.USERNAME%></span><span id="payStatus"></span></div>
                </li> <%}%>
            </ul>--%>

              <iframe id="iframe_Filelist" scrolling="no" frameborder="0" style="width: 100%; height: 1000px; margin-top:20px;">
                    </iframe>
        </div>
