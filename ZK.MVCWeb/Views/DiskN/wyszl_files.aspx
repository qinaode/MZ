...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function LoadData(){
            $.ajax({
                url: "/diskn/CollectionFileList?pid="+pid,
                type: 'post',
                async: false,
        ...
        function UploadStart(file) {
            upload.uploadify('settings', 'uploader', "/diskn/UploadFile?uid="+<%=ViewData["uid"] %>+"&pid="+pid+"&uptype=wyszl");
        ...
        function Update(ids,value,callback) {
            $.ajax({
                ...
                type: 'post',
                data: { ids: ids, value: value,type:"wyszl" },
                success: function (data) {
...
//           }
            //返回到文件列表
             window.location.href="/diskn/wyszl_folder";
       ...
       function GetDataList(id){
         ...
         if (row&&(row.ft==1||row.ft==2||row.ft==3)) {
            pid=row.id;
            var url="/diskn/CollectionFileList?pid="+pid;
            urls.push(url);
            datagrid.Reload(url); 
         }else {
            //只支持单文件下载
            window.location.href = "/diskn/download?type=szl&id="+row.id;
         }
    ...
    function EndCollection(){
        ...
        if(l==0){
            ...
            $.ajax({
                url:'/diskn/EndCollection',
                type:'post',
                data:{folderid:rows[0].id},
                success:function(){
                     tip.Tip({ icon: "ok", msg: "结束收集成功！" });
                     datagrid.Refresh();
                },
                error:function(){
                    tip.Tip({ icon: "error", msg: "结束收集失败！" });
                }
            });
        }
    }
