...
<title></title>
    ...
    <script src="/js/DiskNJS/NSharePage.js" type="text/javascript"></script>
    <script type="text/javascript">
        ...
        function Ok() {
            ...
            //将用户ids和文件ids传到后台进行权限设置
            var fids = '<%=ViewData["fids"] %>';
            uids = uids.join(",");
            $.ajax({
                url: '/diskn/SetUsers',
                type: 'post',
                data: { uids: uids, fids: fids },
                success: function (data) {
                    if (data == "ok") {
                        // parent.CollectionFolderList();
                        //parent.$.messager.alert('提示', '权限设置成功');
                        parent.tip.Tip({ icon: "ok", msg: "权限设置成功！" });
                        parent.datagrid.Refresh();
                        parent.$.modalDialog.handler.dialog('close'); //关闭窗口

                    } else {
                        //parent.tip.Tip({ icon: "error", msg: "权限设置失败！" });
                        alert("权限设置失败！");
                    }
                },
                error: function () {
                    // parent.tip.Tip({ icon: "error", msg: "权限设置错误！" });
                    alert("权限设置失败");
                }
            });
        }
...
<body onload="MM_preloadImages('/css/DiskN/images/icon4.png')">
    <div class="gxk">
        ...
        <div class="right" >
            ...
            <div class="footer">
                <div class="queding">
                    <a href="javascript:void(0)" onclick="Ok()">立即设置</a>
                </div>
                <div class="quxiao">
