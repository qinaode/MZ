...
function GetDataForPaging(pageindex, PagerDivID) {
    $.ajax({
        type: "Post",
        url: "../ashx/MoralManage_Resource.ashx?_a=" + Math.random(),
        data: { "Flag": "GetMoralResourceListPaging", "strWhere": strWhere,"channelGroupName":channelGroupName, "PageIndex": pageindex, "PageSize": pagesize },
        datatype: "text/json",
        success: function (backdata) {
            ...
            $("#div_ResourceContent").setTemplateURL("../pagetemples/MoralManagement/moralresourcelist.htm", null, null);
...
//绑定条件查询列表
function GetMoralResourceListByCondition() {
    strWhere = $("#text_MoralResourceName").val();
...
function SendCheckedItems_AdminResource(btn_id) {
    ...
    //        return false;
    //    };
    $.dialog({ title: '添加资源到专题', width: '390px', height: '160px', content: 'url:MoralResourceSendEdit.aspx?tyid=' + fileid + '&t=' + new Date().getTime().toString(), max: false, min: false });
}
