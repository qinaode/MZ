<%@ Page Title="" Language="C#" MasterPageFile="~/ZKManage.Master" AutoEventWireup="true"
    CodeBehind="MoralManagementCategory.aspx.cs" Inherits="ZK.Manage.MoralManagement.MoralManagementCategory" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphJs" runat="server">
    ...
    <script src="../inc/dtree_Moral.js" type="text/javascript"></script>
    <%--<script src="../js/adminisstrationJs.js" type="text/javascript"></script>--%>
    <script language="javascript" type="text/jscript">
        function moralManagementCategoryAdd() {
            //添加  cancel: true, , ok: function () { Save_UpdateOrAdd(); }
            $.dialog({ title: '添加德育频道', width: '390px', height: '150px', content: 'url:MoralCategoryEdit.aspx?ty=add&t=' + new Date().getTime().toString(), max: false, min: false });

        }

        function childCateAdd(id) {
            $.dialog({ title: '添加子类德育频道', width: '390px', height: '150px', content: 'url:MoralCategoryEdit.aspx?ty=addchild&t=' + new Date().getTime().toString() + '&id=' + id, max: false, min: false });

        }
        function childCateEdit(id) {
            $.dialog({ title: '编辑德育频道', width: '390px', height: '150px', content: 'url:MoralCategoryEdit.aspx?ty=edit&t=' + new Date().getTime().toString() + '&id=' + id, max: false, min: false });
...
<asp:Content ID="Content3" ContentPlaceHolderID="cphContent" runat="server">
    <div class="content">
        <form action="MoralManagementCategory.aspx" runat="server" id="queryfrom" name="queryfrom">
        <input type="hidden" name="orderKey" id="orderKey" value="ID" />
        <div class="pagePath">
            首页 >频道管理>德育分类管理</div>
           <div class="b d1">
            <ul>
                <li>可创建多级的德育分类列表，点击一个“德育分类”可修改德育分类名称。</li>
                <li><span style="color: #2fb900">提示：德育分类列表更改后，客户端需要重新登录才能刷新。</span></li>
            </ul>
        </div>
        <div class="searchform" >
          <%--  分类管理名称：
            ...
            <input type="button" value="添加" style="font-family:微软雅黑; font-size:9;" onclick="moralManagementCategoryAdd();" /></p>
