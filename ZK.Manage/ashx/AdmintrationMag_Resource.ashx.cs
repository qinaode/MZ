...
{
    /// <summary>
    /// AdmintrationMag_Resource 的摘要说明
    /// </summary>
    public class AdmintrationMag_Resource : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (Flag)
            {
                case "GetAdminResourceListPaging":
                    Result = GetAdminResourceListPaging(context);
                    break;
                case "GetAdminResourceList":
                    Result = GetAdminResourceList(context);
                    break;
                case "DeleteCheckedAdminResource":
                    Result = DeleteCheckedAdminResource(context);
                    break;
        ...
        private string GetAdminResourceListPaging(HttpContext context)
        {
            ...
            //builder.Append(" and ownerID=" + UserID);
        ...
        private string GetAdminResourceList(HttpContext context)
        {
            List<ZK.Model.ZK_FileList> List = new List<Model.ZK_FileList>();
            string UserID = "1";
        ...
        private string DeleteCheckedAdminResource(HttpContext context)
        {
        ...
        {
            //string strWhere = " fileID in (select fileID from ZK_ChannelGroupAndFileList where channelGroupID in (select a.channelGroupID from ZK_ChannelGroupAndFileList a left join ZK_ChannelGroup b on a.channelGroupID=b.channelGroupID left join ZK_FileList c on a.fileID=c.fileID where b.channelID=3))";
        ...
        private string GetchannelGroupIDsByCondition(string channelgroupname)
        {
            ...
            string strWhere = " channelGroupID in (select a.channelGroupID from ZK_ChannelGroupAndFileList a left join ZK_ChannelGroup b on a.channelGroupID=b.channelGroupID left join ZK_FileList c on a.fileID=c.fileID where b.channelID=3)";
            if (channelgroupname != "")
            {
