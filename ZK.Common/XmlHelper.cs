...
namespace ZK.Common
{
    ...
    public class XMLHelper
    {
        ...
        /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名")</param>
        /// <returns>返回XmlNodeList</returns>
        public static List<XmlNode> GetXmlNodeListByXpathAndAttr(string xmlFileName, string xpath, string xmlattr, string value)
        {
            XmlDocument xmlDoc = new XmlDocument();
            ...
            try
            {
                ...
                {
                    string filename = item.Attributes[xmlattr].Value;
                    int fileindex = filename.LastIndexOf(".");
                    string new_name = filename.Substring(0,fileindex);
                    //if ((item.Attributes[xmlattr].Value) == value)
                    if (new_name==value)
                    {
                        lists.Add(item);
                    }
        ...
        /// <param name="xmlNodeName">要匹配xmlNodeName的节点名称</param>
        /// <param name="innerText">节点文本值</param>
        /// <param name="xmlAttributeName">要匹配xmlAttributeName的属性名称集合</param>
        /// <param name="value">属性值集合</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateXmlNodeByXPath(string xmlFileName, string xpath, string xmlNodeName, string innerText, List<string> xmlAttributeName, List<string> value)
        {
            bool isSuccess = false;
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    //如果属性和值参数都不为空则在此新节点上新增属性
                    for (int i = 0; i < xmlAttributeName.Count; i++)
                    {
        ...
        /// <summary>
        /// 批量  依据匹配XPath表达式的第一个节点来创建或更新它的属性(如果属性存在则更新,不存在则创建)
        /// </summary>
        /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
        /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名</param>
        /// <param name="xmlselectName">要查找的节点的标志属性名</param>
        /// <param name="xmlselectValue">要查找的节点的标志属性对应值</param>
        /// <param name="xmlAttributeName">要匹配xmlAttributeName的属性名称集合</param>
        /// <param name="value">属性值集合</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateOrUpdateXmlAttributeByXPath(string xmlFileName, string xpath, string xmlselectName, string xmlselectValue, List<string> xmlAttributeName, List<string> value)
        {
            bool isSuccess = false;
            bool isExistsAttribute = false;//标识属性是否存在
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    {
                        //找到相应的节点
                        if (item.Attributes[xmlselectName].Value == xmlselectValue)
                        {
                            //循环该节点的所有属性
                            foreach (XmlAttribute attribute in item.Attributes)
                            {
                                ...
                                for (int i = 0; i < xmlAttributeName.Count; i++)
                                {
                                    if (attribute.Name.ToLower() == xmlAttributeName[i].ToLower())
                                    {
                                        //节点中存在此属性则更新
        ...
        /// <summary>
        /// 无重名标签时可用 依据匹配XPath表达式的第一个节点来创建或更新它的属性(如果属性存在则更新,不存在则创建)
        /// </summary>
        /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
        /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名</param>
        /// <param name="xmlAttributeName">要匹配xmlAttributeName的属性名称</param>
        ...
        /// <summary>
        /// 删除匹配XPath表达式的特定元素节点(节点中的子元素同时会被删除)
        /// </summary>
        /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
        /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名</param>
        /// <param name="attr">要寻找的node的属性名</param>
        /// <param name="value">该属性名的值</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool DeleteXmlNodeByXPath(string xmlFileName, string xpath, string attr, string value)
        {
            bool isSuccess = false;
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    foreach (XmlNode item in xmlNode)
                    {
                        if (item != null && item.Attributes[attr].Value == value)
                        {
