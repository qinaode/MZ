...
namespace ZK.BLL
{
	...
	{
		private readonly Iminiyun_file_exifs dal=DataAccess.Createminiyun_file_exifs();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.miniyun_file_exifs GetModelByCache(int id)
		{
			
			string CacheKey = "miniyun_file_exifsModel-" + id;
			...
			return (ZK.Model.miniyun_file_exifs)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_file_exifs> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_file_exifs> DataTableToList(DataTable dt)
		{
			List<ZK.Model.miniyun_file_exifs> modelList = new List<ZK.Model.miniyun_file_exifs>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.miniyun_file_exifs model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.miniyun_file_exifs();
					...
					if(dt.Rows[n]["longtitude"]!=null && dt.Rows[n]["longtitude"].ToString()!="")
					{
					model.longtitude=dt.Rows[n]["longtitude"].ToString();
					}
					if(dt.Rows[n]["exif"]!=null && dt.Rows[n]["exif"].ToString()!="")
					{
					model.exif=dt.Rows[n]["exif"].ToString();
					}
