...
namespace ZK.BLL
{
	...
	{
		private readonly Iminiyun_file_tags dal=DataAccess.Createminiyun_file_tags();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.miniyun_file_tags GetModelByCache(int id)
		{
			
			string CacheKey = "miniyun_file_tagsModel-" + id;
			...
			return (ZK.Model.miniyun_file_tags)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_file_tags> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_file_tags> DataTableToList(DataTable dt)
		{
			List<ZK.Model.miniyun_file_tags> modelList = new List<ZK.Model.miniyun_file_tags>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.miniyun_file_tags model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.miniyun_file_tags();
					...
					if(dt.Rows[n]["tag_id"]!=null && dt.Rows[n]["tag_id"].ToString()!="")
					{
						model.tag_id=int.Parse(dt.Rows[n]["tag_id"].ToString());
					}
					modelList.Add(model);
				}
			}
