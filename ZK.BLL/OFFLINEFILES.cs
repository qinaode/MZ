...
namespace ZK.BLL
{
	...
	{
		private readonly IOFFLINEFILES dal=DataAccess.CreateOFFLINEFILES();
		public OFFLINEFILES()
		{}
		#region  Method
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.OFFLINEFILES GetModelByCache(string SID)
		{
			
			string CacheKey = "OFFLINEFILESModel-" + SID;
			...
			return (ZK.Model.OFFLINEFILES)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.OFFLINEFILES> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.OFFLINEFILES> DataTableToList(DataTable dt)
		{
			List<ZK.Model.OFFLINEFILES> modelList = new List<ZK.Model.OFFLINEFILES>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.OFFLINEFILES model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.OFFLINEFILES();
					...
					if(dt.Rows[n]["RECEIVER_USERID"]!=null && dt.Rows[n]["RECEIVER_USERID"].ToString()!="")
					{
						model.RECEIVER_USERID=int.Parse(dt.Rows[n]["RECEIVER_USERID"].ToString());
					}
