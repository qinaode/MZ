...
namespace ZK.BLL
{
	...
	{
		private readonly Iminiyun_events dal=DataAccess.Createminiyun_events();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.miniyun_events GetModelByCache(int id)
		{
			
			string CacheKey = "miniyun_eventsModel-" + id;
			...
			return (ZK.Model.miniyun_events)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_events> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_events> DataTableToList(DataTable dt)
		{
			List<ZK.Model.miniyun_events> modelList = new List<ZK.Model.miniyun_events>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.miniyun_events model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.miniyun_events();
					...
					if(dt.Rows[n]["user_device_id"]!=null && dt.Rows[n]["user_device_id"].ToString()!="")
					{
						model.user_device_id=int.Parse(dt.Rows[n]["user_device_id"].ToString());
					}
