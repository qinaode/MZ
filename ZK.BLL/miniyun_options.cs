...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2013/10/30 10:55:29   N/A    初版
*
...
namespace ZK.BLL
{
	...
	{
		private readonly Iminiyun_options dal=DataAccess.Createminiyun_options();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.miniyun_options GetModelByCache(int option_id)
		{
			
			string CacheKey = "miniyun_optionsModel-" + option_id;
			...
			return (ZK.Model.miniyun_options)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_options> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_options> DataTableToList(DataTable dt)
		{
			List<ZK.Model.miniyun_options> modelList = new List<ZK.Model.miniyun_options>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.miniyun_options model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.miniyun_options();
					if(dt.Rows[n]["option_id"]!=null && dt.Rows[n]["option_id"].ToString()!="")
					{
						model.option_id=int.Parse(dt.Rows[n]["option_id"].ToString());
					}
					if(dt.Rows[n]["option_name"]!=null && dt.Rows[n]["option_name"].ToString()!="")
					{
					model.option_name=dt.Rows[n]["option_name"].ToString();
					}
					if(dt.Rows[n]["option_value"]!=null && dt.Rows[n]["option_value"].ToString()!="")
					{
					model.option_value=dt.Rows[n]["option_value"].ToString();
					}
