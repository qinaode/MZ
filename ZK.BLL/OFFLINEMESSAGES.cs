...
namespace ZK.BLL
{
	...
	{
		private readonly IOFFLINEMESSAGES dal=DataAccess.CreateOFFLINEMESSAGES();
		...
		{
			return dal.Exists(MSGID,USERID);
		}
		...
		public bool Delete(string MSGID,int USERID)
		{
			...
			return dal.Delete(MSGID,USERID);
		}
		...
		public ZK.Model.OFFLINEMESSAGES GetModel(string MSGID,int USERID)
		{
			...
			return dal.GetModel(MSGID,USERID);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.OFFLINEMESSAGES GetModelByCache(string MSGID,int USERID)
		{
			
			string CacheKey = "OFFLINEMESSAGESModel-" + MSGID+USERID;
			object objModel = ZK.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(MSGID,USERID);
					if (objModel != null)
					{
			...
			return (ZK.Model.OFFLINEMESSAGES)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.OFFLINEMESSAGES> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.OFFLINEMESSAGES> DataTableToList(DataTable dt)
		{
			List<ZK.Model.OFFLINEMESSAGES> modelList = new List<ZK.Model.OFFLINEMESSAGES>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.OFFLINEMESSAGES model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.OFFLINEMESSAGES();
