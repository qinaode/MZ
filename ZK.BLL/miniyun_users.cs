...
namespace ZK.BLL
{
	...
	{
		private readonly Iminiyun_users dal=DataAccess.Createminiyun_users();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.miniyun_users GetModelByCache(int id)
		{
			
			string CacheKey = "miniyun_usersModel-" + id;
			...
			return (ZK.Model.miniyun_users)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_users> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.miniyun_users> DataTableToList(DataTable dt)
		{
			List<ZK.Model.miniyun_users> modelList = new List<ZK.Model.miniyun_users>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.miniyun_users model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.miniyun_users();
					if(dt.Rows[n]["id"]!=null && dt.Rows[n]["id"].ToString()!="")
					{
						model.id=int.Parse(dt.Rows[n]["id"].ToString());
					}
					if(dt.Rows[n]["user_uuid"]!=null && dt.Rows[n]["user_uuid"].ToString()!="")
					{
					model.user_uuid=dt.Rows[n]["user_uuid"].ToString();
					}
					if(dt.Rows[n]["user_name"]!=null && dt.Rows[n]["user_name"].ToString()!="")
					{
					model.user_name=dt.Rows[n]["user_name"].ToString();
					}
					if(dt.Rows[n]["user_pass"]!=null && dt.Rows[n]["user_pass"].ToString()!="")
					{
					model.user_pass=dt.Rows[n]["user_pass"].ToString();
					}
					if(dt.Rows[n]["user_status"]!=null && dt.Rows[n]["user_status"].ToString()!="")
					{
                        string str = dt.Rows[n]["user_status"].ToString();
