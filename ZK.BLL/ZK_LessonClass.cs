...
namespace ZK.BLL
{
	...
	{
		private readonly IZK_LessonClass dal=DataAccess.CreateZK_LessonClass();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.ZK_LessonClass GetModelByCache(int classID)
		{
			
			string CacheKey = "ZK_LessonClassModel-" + classID;
			...
			return (ZK.Model.ZK_LessonClass)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.ZK_LessonClass> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.ZK_LessonClass> DataTableToList(DataTable dt)
		{
			List<ZK.Model.ZK_LessonClass> modelList = new List<ZK.Model.ZK_LessonClass>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.ZK_LessonClass model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.ZK_LessonClass();
