...
namespace ZK.BLL
{
	...
	{
		private readonly IView_OtherFileList dal=DataAccess.CreateView_OtherFileList();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ZK.Model.View_OtherFileList GetModelByCache()
		{
			//该表无主键信息，请自定义主键/条件字段
			string CacheKey = "View_OtherFileListModel-" ;
			...
			return (ZK.Model.View_OtherFileList)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.View_OtherFileList> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ZK.Model.View_OtherFileList> DataTableToList(DataTable dt)
		{
			List<ZK.Model.View_OtherFileList> modelList = new List<ZK.Model.View_OtherFileList>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ZK.Model.View_OtherFileList model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ZK.Model.View_OtherFileList();
