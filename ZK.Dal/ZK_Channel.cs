...
namespace ZK.Dal
{
	...
	/// 数据访问类:ZK_Channel
	/// </summary>
	public partial class ZK_Channel:IZK_Channel
	{
		public ZK_Channel()
		{}
		#region  Method
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("channelID", "ZK_Channel"); 
		}
		...
		public bool Exists(int channelID)
		{
			...
			strSql.Append("select count(1) from ZK_Channel");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into ZK_Channel(");
			strSql.Append("channelName,channelDesc)");
			strSql.Append(" values (");
			strSql.Append("@channelName,@channelDesc)");
			strSql.Append(";select @@IDENTITY");
			SqlParameter[] parameters = {
					new SqlParameter("@channelName", SqlDbType.NVarChar,50),
					new SqlParameter("@channelDesc", SqlDbType.NVarChar,50)};
			parameters[0].Value = model.channelName;
			parameters[1].Value = model.channelDesc;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update ZK_Channel set ");
			strSql.Append("channelName=@channelName,");
			strSql.Append("channelDesc=@channelDesc");
			strSql.Append(" where channelID=@channelID");
			SqlParameter[] parameters = {
					new SqlParameter("@channelName", SqlDbType.NVarChar,50),
					new SqlParameter("@channelDesc", SqlDbType.NVarChar,50),
		...
		public bool Delete(int channelID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ZK_Channel ");
			strSql.Append(" where channelID=@channelID");
			SqlParameter[] parameters = {
					new SqlParameter("@channelID", SqlDbType.Int,4)
			};
		...
		public bool DeleteList(string channelIDlist )
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ZK_Channel ");
			strSql.Append(" where channelID in ("+channelIDlist + ")  ");
		...
		public ZK.Model.ZK_Channel GetModel(int channelID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 channelID,channelName,channelDesc from ZK_Channel ");
			strSql.Append(" where channelID=@channelID");
			SqlParameter[] parameters = {
					new SqlParameter("@channelID", SqlDbType.Int,4)
			};
			...
			ZK.Model.ZK_Channel model=new ZK.Model.ZK_Channel();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["channelDesc"]!=null && ds.Tables[0].Rows[0]["channelDesc"].ToString()!="")
				{
					model.channelDesc=ds.Tables[0].Rows[0]["channelDesc"].ToString();
				}
				return model;
			}
			else
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select channelID,channelName,channelDesc ");
			strSql.Append(" FROM ZK_Channel ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" channelID,channelName,channelDesc ");
			strSql.Append(" FROM ZK_Channel ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			{
				strSql.Append("order by T.channelID desc");
			}
			strSql.Append(")AS Row, T.*  from ZK_Channel T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "*";
			parameters[1].Value = "ZK_Channel";
			parameters[2].Value = strWhere;
			parameters[3].Value = "channelID";
			parameters[4].Value = "channelID desc";
