...
{
	/// <summary>
	/// 数据访问类:ZK_LessonAndFileList
	/// </summary>
	public partial class ZK_LessonAndFileList:IZK_LessonAndFileList
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "ZK_LessonAndFileList"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ZK_LessonAndFileList");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(ZK.Model.ZK_LessonAndFileList model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into ZK_LessonAndFileList(");
			strSql.Append("lessonID,fileID,CategoryId)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(ZK.Model.ZK_LessonAndFileList model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update ZK_LessonAndFileList set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ZK_LessonAndFileList ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public ZK.Model.ZK_LessonAndFileList GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,lessonID,fileID,CategoryId from ZK_LessonAndFileList ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			ZK.Model.ZK_LessonAndFileList model=new ZK.Model.ZK_LessonAndFileList();
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,lessonID,fileID,CategoryId ");
			strSql.Append(" FROM ZK_LessonAndFileList ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,lessonID,fileID,CategoryId ");
			strSql.Append(" FROM ZK_LessonAndFileList ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from ZK_LessonAndFileList T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "*";
			parameters[1].Value = "ZK_LessonAndFileList";
